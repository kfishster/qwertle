...
export class Guess {
	...
	constructor(guess: string, solution: string) {
		this.guess = guess;
		this.substrings = this.findLargestSubstring(guess, solution).map(
			(sub) => new LetterSubstring(guess, sub, false)
		);

		let reverseSolution = solution.split("").reverse().join("");
		this.reverseSubstrings = this.findLargestSubstring(
			guess,
			reverseSolution
		).map((sub) => new LetterSubstring(guess, sub, true));
		...
		this.reverseSubstrings.forEach((sub) => {
			Array.from(Array(sub.length).keys()).map((j) =>
				substringLetterIdxs.push(sub.idx + j)
			);
	...
	public findLargestSubstringRec(
		guess: string,
		solution: string,
		substringsChecked: string[]
	): string[] {
		...
		const endSubs = this.findLargestSubstringRec(
			guess.substring(0, guess.length - 1),
			solution,
			substringsChecked
		);
		const startSubs = this.findLargestSubstringRec(
			guess.substring(1, guess.length),
			solution,
			substringsChecked
		);

...
export function getDefaultGameState(practice: boolean): GameState {
	const hash = practice
		? Math.floor(Math.random() * wordleSolutions.length)
		: getHashForDate();
...
export function cluesUsed(settings: GameSettings): number {
	return [settings.showKeyboardHeatmap, settings.showSubstrings].filter(Boolean)
		.length;
}
...
export function validateGuess(gameGuess: string): boolean {
	...
	return (
		wordleSolutions.includes(lowerWord) || wordleDictionary.includes(lowerWord)
	);
}

export const addLetterToGameState = (
	game: GameState,
	letter: string
): GameState => {
	if (
		!isGameFinished(game.status) &&
		game.currentGuess.length < Constants.wordLength
	) {
		return {
			...game,
...
export const submitGuessToAppState = (appState: AppState): AppState => {
	if (
		!isGameFinished(appState.game.status) &&
		appState.game.guesses.length < appState.settings.numGuesses
	) {
		...
		if (validateGuess(guessWord)) {
			...
			const newGameState = submitGuessToGameState(
				appState.game,
				guessWord,
				appState.settings.numGuesses
			);
			...
			return {
				...
				modals: {
					...appState.modals,
					resultsOpen: isGameFinished(newGameState.status)
						? true
						: appState.modals.resultsOpen,
				},
			};
		} else {

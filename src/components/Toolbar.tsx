    ...
    return (
        <div className="flex flex-col text-mainframe-green justify-between items-center w-full px-2">
            <div className="flex flex-row text-mainframe-green justify-between items-start w-full">
                ...
                <div className={`flex basis-1/2 justify-center flex-row gap-4 min-w-screen font-mono tracking-wider font-bold transition-all ease-out duration-700`} onAnimationEnd={finishedAnimation}>
                    <div className="">
            ...
            <div className="flex flex-row my-4 gap-4">
                <div className="flex flex-col h-4 items-center">
                    <Toggle aria-label="Practice" defaultChecked={practice} onChange={togglePractice} icons={false}/>
                    {gameFinished && <RefreshIcon className="flex h-8 stroke-1" />}
                    <p className="text-xs mt-1">{practice ? "practice" : "daily"}</p>
                </div>
                {practice && gameFinished && <button><RefreshIcon className="flex h-8 stroke-1 hover:animate-spin" onClick={refreshPractice}/></button>}
            </div>
        </div>
    );
};
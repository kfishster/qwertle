...
function getLetterColor(
	letterGuess: LetterGuess | undefined,
	currentGuessLetter: string | undefined
) {
...
export const BoardCell = ({
	...
	return (
		...
		<div
			className={`flex ${
				letterGuess && !insideSubstring ? "grow-[.2]" : "flex-1"
			} aspect-square ${
				letterGuess && "transition-colors ease-out duration-1000"
			} justify-center items-center box-border ${
				smallDemo
					? "h-8 w-8 border-2 text-xl"
					: "border-2 text-2xl md:text-3xl lg:text-4xl"
			} font-mono border-dashed rounded-sm font-semibold ${
				Constants.borderColor
			} ${getLetterBackground(letterGuess)} ${getLetterColor(
				letterGuess,
				currentGuessLetter
			)}`}
		>

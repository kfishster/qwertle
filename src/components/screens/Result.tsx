...
function createShareMessage(
  ...
  return [
    `QWERTLE ${practice ? "" : `#${qwordleNum} `}${
      isWinner ? guesses.length : "X"
    }/${numGuesses}${Array.from(Array(cluesUsed).keys())
      .map((c) => "*")
      .join("")}`,
    ...
    <Modal close={closeResult}>
      <div className="flex flex-col items-center gap-2 w-full">
        ...
        <div className="pb-8">
          <BoardRow
            smallDemo={true}
            guess={new Guess(game.solution, game.solution)}
            showSubstrings={false}
            gameFinished={true}
          />
        </div>
        <div className="flex flex-col items-center pb-4 gap-1">
          {shareMessage.map((l, i) => (
            <p key={i} className={`${i !== 0 && `h-4`}`}>
              {l}
            </p>
          ))}
        </div>
